...
SRC_ROOT="/Users/luxiaoyue/Documents/lxy"
...
mkdir -p "$REPO_DIR/report_paper" "$REPO_DIR/openclaw"

# Mirror sibling folders to repo root siblings
rsync -a --delete --exclude '.git/' --exclude '.DS_Store' \
  "$SRC_ROOT/report_paper/" "$REPO_DIR/report_paper/"
rsync -a --delete --exclude '.git/' --exclude '.DS_Store' \
  "$SRC_ROOT/openclaw/" "$REPO_DIR/openclaw/"

status_out=$(git -C "$REPO_DIR" status --porcelain)
if [[ -n "$status_out" ]]; then
  git -C "$REPO_DIR" add -A
  git -C "$REPO_DIR" commit -m "sync report_paper + openclaw folders $(date "+%Y-%m-%d %H:%M:%S")"
